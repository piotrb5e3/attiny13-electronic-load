...
static inline void set_adc_read_real_current() {
    ADMUX |= 0x03;
}
static inline void set_adc_read_set_current() {
    ADMUX = (ADMUX & (~0x01)) | 0x02;
}
...
int main() {
    ...
    uint8_t reading_real = 1;

    while (1) {
        if (reading_real) {
            led_green();
            set_adc_read_real_current();
        } else {
            led_red();
            set_adc_read_set_current();
        }
        for (uint8_t i = 0; i < 20; i++) {
            uint16_t val = get_current();
            // Skip displaying real current if it's equal to 0
            if (val == 0 && reading_real) {
                break;
            }
            show_number(val);
            _delay_ms(300);
        }
        reading_real = !reading_real;
    }
}
